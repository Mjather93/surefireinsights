...
class ProcessPerformanceExtractor:
    ...
    def get_cpu_usage(self, process_info):
        try:
            ...
            return (self.get_current_time(), process_info['pid'], process_info['name'],
    ...
    def get_memory_usage(self, process_info):
        try:
            ...
            return (self.get_current_time(), process_info['pid'],
                    process_info['name'],
    ...
    @staticmethod
    def insert_into_database(data, report_fk):
        conn = sqlite3.connect('surefireinsights.db')
        cursor = conn.cursor()
        for item in data:
            print(tuple(item) + (report_fk,))
            cursor.execute('''INSERT INTO process_metrics (
                            timestamp,process_id,process_name,process_command_line,
                            metric_name,metric_value,report_fk,
                           )
                           VALUES (?, ?, ?, ?, ?, ?, ?)''', tuple(item) + (report_fk,))
        conn.commit()
        conn.close()
